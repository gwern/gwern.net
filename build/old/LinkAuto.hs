...
Date: 2021-06-23
When:  Time-stamp: "2026-04-26 11:31:48 gwern"
License: CC-0
...
import Text.Pandoc (topDown, nullAttr, readerExtensions, def, writeHtml5String, pandocExtensions, runPure, readHtml, Pandoc(..), Inline(Link,Image,Code,Span,Str,Quoted,Space), nullMeta, Block(Para), QuoteType(DoubleQuote), Pandoc(..))
...
linkAutoTest = map (\(inline', doc') -> (inline', doc', linkAutoInline2Doc inline')) $ filter (\(inline, doc) -> linkAutoInline2Doc inline /= doc) linkAutoTests
...
customDefinitions subsetter = if length customSorted > 1007 then error ("LinkAuto.hs (customDefinitions): 'customSorted' too long (" ++ show (length customSorted) ++ "), which will trigger the LA slowdown, making site compiles unacceptably slow. Delete some unused regexp rewrite rules!")
                              else customDefinitionsR $ definitionsValidate $ subsetter customSorted -- delimit & compile
...
custom = setLike [
        ...
        , ("([Ccc]hain[ -][Oo]f[ -][Tt]hought|CoT)", "https://arxiv.org/abs/2201.11903#google") -- Chain-of-thought/Chain-of-Thought/chain-of-thought/CoT inner-monologue variant
