...
hyphenateInline x@(Str s) = if T.any (=='\173') s then x else -- U+00AD SOFT HYPHEN, HTML: &#173; &shy;
                              Str $ T.replace "-\173" "-" $ -- odd edge-case exposed on Safari: because hyphenator breaks on hyphens (why?), interspersing the soft hyphen results in *two* hyphens being displayed should the line break there! since the regular hyphen already ensures a linbreak opportunity
