...
# Date: 2020-02-07
# When:  Time-stamp: "2022-04-08 10:24:15 gwern"
# License: CC-0
#
...
USER_AGENT="Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:96.0) Gecko/20100101 Firefox/96.1"
    ...
    if [[ "$HTTP_STATUS" == "404" ]]; then
        ...
        then
            ...
            then
                ...
                chromium --temp-profile "./docs/www/$DOMAIN/$HASH.pdf"
            else
                echo "Unsuccessful: $1 $HASH" 1>&2
                exit 1
            fi
            ...
            if [[ -f "$TARGET" ]]; then
                ...
                if [[ -z "$ERROR_404" ]]; then
                    ...
                    chromium --temp-profile "./docs/www/$DOMAIN/$HASH.html$ANCHOR" "$1" &
                else
                    rm "$TARGET"
                    echo "Unsuccessful: $1 $HASH" 1>&2
                    exit 1
