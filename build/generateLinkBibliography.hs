...
-- long items:
generateLinkBibliographyItem am pathParent (f,ident,mi) = let prefix = if null ident then [] else [Link ("",["id-not", "collapsible"],[]) [Str "\8203"] ((T.pack $ (takeWhile (/='#') pathParent) ++ "#" ++ ident), "Original context in page."), Space] in
