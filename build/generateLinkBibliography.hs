...
generateLinkBibliographyItem _ pathParent (f,ident,(t,_,_,_,_,_,""))  = -- short:
  let f'
      ...
      prefix = if null ident then [] else [Link ("",["id-not"],[]) [Str "\8203"] ((T.pack $ pathParent ++ "#" ++ ident), "Original context in page."), Space] -- ZERO WIDTH SPACE to make clear that 'this link intentionally left blank';
...
-- long items:
generateLinkBibliographyItem am pathParent (f,ident,mi) = let prefix = if null ident then [] else [Link ("",["id-not"],[]) [Str "\8203"] ((T.pack $ pathParent ++ "#" ++ ident), "Original context in page."), Space] in
