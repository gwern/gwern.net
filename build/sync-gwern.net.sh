...
# Date: 2016-10-01
# When:  Time-stamp: "2022-11-07 12:13:02 gwern"
# License: CC-0
#
...
then
  ...
  if [ "$SLOW" ]; then
    ...
    λ(){ find metadata/annotations/ -name "*.html" \
             ...
             grep -E -v -e '^annotation-see-also$' -e '^archive-local$' -e '^archive-not$' -e '^author$' -e '^aux-links$' -e '^backlink-not$' \
                   ...
                   -e '^mjx-vsize$' -e '^new$' -e '^outline-not$' -e '^warning$' -e '^markdown-body$' -e '^similars$' -e '^similars-append$' -e '^text-center$' -e '^abstract-tag-directory$' -e '^page-description-annotation$'; }
    ...
    λ(){ find ./ -type f -name "*.page" | grep -F --invert-match '_site' | sort | sed -e 's/\.page$//' -e 's/\.\/\(.*\)/_site\/\1/' | \
             xargs --max-args=100 grep -F --with-filename --color=always -e ")'s " -e "}'s " -e '">?' | \
