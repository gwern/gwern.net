...
import Data.RPTree (knn, forest, rpTreeCfg, fpMaxTreeDepth, fpDataChunkSize, fpProjNzDensity, fromListDv, DVector, Embed(..), RPForest)
...
foo :: Embeddings -> RPForest Double (V.Vector (Embed DVector Double String))
foo es = let minLeafSize = 2 -- ???
             ...
             nTrees = 10 -- ???
             projectionVectorDimension = 3 -- ???
         in
         runIdentity $ forest 0 (fpMaxTreeDepth cfg) minLeafSize nTrees (fpDataChunkSize cfg) (fpProjNzDensity cfg) projectionVectorDimension $ bar es

bar :: Embeddings -> ConduitT () (Embed DVector Double String) Identity ()
bar = sourceList . map (\(p,_,embedding) -> Embed (fromListDv embedding) p)

{-
> edb <- readEmbeddings
> let f = foo edb
> knn inner 1 f ((fromListDv $ take 512 $ repeat 0.5)::DVector Double)
