...
module LinkMetadata (addPageLinkWalk, isPagePath, readLinkMetadata, readLinkMetadataAndCheck, walkAndUpdateLinkMetadata, updateGwernEntries, writeAnnotationFragments, Metadata, MetadataItem, MetadataList, readYaml, readYamlFast, writeYaml, annotateLink, createAnnotations, hasAnnotation, parseRawBlock, parseRawInline, generateID, generateURL, generateAnnotationBlock, generateAnnotationTransclusionBlock, getSimilarLink, authorsToCite, authorsTruncate, cleanAbstractsHTML, sortItemDate, sortItemPathDate, warnParagraphizeYAML, simplifiedHTMLString, tooltipToMetadata, dateTruncateBad, urlToAnnotationPath) where
...
import Utils (writeUpdatedFile, printGreen, printRed, fixedPoint, currentYear, sed, sedMany, replaceMany, toMarkdown, trim, simplified, anyInfix, anyPrefix, anySuffix, replace, split, anyPrefixT, hasAny, safeHtmlWriterOptions, addClass)
...
generateAnnotationBlock truncAuthorsp annotationP (f, ann) blp slp lb = case ann of
                              ...
                              Just (tle,aut,dt,doi,ts,abst) ->
                                let tle' = if null tle then "<code>"++f++"</code>" else tle
                                    ...
                                    lidLinkBibLinkFragment = if lid=="" then "" else "linkbibliography-transclusion-" `T.append` lid
