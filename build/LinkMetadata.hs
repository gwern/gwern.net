...
Date: 2019-08-20
When:  Time-stamp: "2020-12-14 17:37:29 gwern"
License: CC-0
-}
...
pdf p = do (_,_,mb) <- runShellCommand "./" Nothing "exiftool" ["-printFormat", "$Title$/$Author$/$Date$/$DOI", "-Title", "-Author", "-Date", "-DOI", p]
           if BL.length mb > 0 then
             do let (etitle:eauthor:edate:edoi:_) = lines $ U.toString mb
                ...
                let author = initializeAuthors $ trim $ if length eauthor > length ecreator || ("Adobe" `isInfixOf` ecreator || "Arbortext" `isInfixOf` ecreator || "Unicode" `isInfixOf` ecreator) then eauthor else ecreator
