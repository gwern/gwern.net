...
generateDirectory newestp am md ldb sortDB dirs dir'' = do
  ...
  abstract <- if not ("doc/" `isPrefixOf` dir'') then return [] else
                do let tagBase = takeDirectory $ last $ splitPath  dir'' -- 'doc/cat/psychology/catnip/' -> 'catnip'
                   ...
                   return $ if abstractp then [Div ("manual-annotation", ["abstract", "abstract-tag-directory", "dropcap-not"], []) [Para [Link ("", ["include-content-core", "include-strict", "link-page"], [])
...
listFiles m direntries' = do
                   ...
                   let files'          = (sort . filter (\f -> not $ anySuffix f ["index", ".tar", ".webm-poster.jpg", ".mp4-poster.jpg", "-poster-large.jpg"]) . map (sed "\\.md$" "") . filter ('#' `notElem`)) files
