...
generateDirectory md dirs dir'' = do
  ...
  let allUnannotatedUntitledP = (length untitledLinks >= 3) && (all (=="") $ map (\(_,(_,_,_,_,_,annotation),_,_) -> annotation) untitledLinks) -- whether to be compact columns
...
generateItem (f,(t,aut,dt,_,tgs,""),bl,sl) = -- no abstracts:
 if ("https://en.wikipedia.org/wiki/"`isPrefixOf`f) then [Para [Link ("",["include-annotation", "include-no-spinner"],[]) [Str "Wikipedia"] (T.pack f, if t=="" then "" else T.pack $ "Wikipedia link about " ++ t)]]
 else
  let
