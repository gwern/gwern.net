...
# Date: 2016-10-01
# When:  Time-stamp: "2026-07-08 11:56:27 gwern"
# License: CC-0
#
  ...
  if [ "$SLOW" ]; then
    ...
    λ(){
        ...
        declare -a html_classes_whitelist=(
            ...
            "image-annotated" "image-annotated-not" "image-annotated-partial" "image-modified-recently" "link-modified-recently" "page-created-recently"
            ...
            "icon-single-white-star-on-black-circle" "icon-manicule-left" "icon-manicule-right"
 ...
 if [ "$SLOW" ]; then
  ...
  if [ "$SLOW" ]; then
    ...
    λ(){ CORRUPT=$(find ./doc/ -type f -mtime -31 -name "*.png" | parallel --jobs "$N" --max-args=500 file | gfv 'PNG image data' | cut --delimiter ':' -f 1)
         if [ -n "$CORRUPT" ]; then
             echo -e "Found broken PNGs:\n$CORRUPT"
             echo "Now attempting to convert the JPG/GIF/PNG ones into PNG…"
             for FILE in $CORRUPT; do
                 if file "$FILE" | grep --fixed-strings --silent -e 'JPEG image data' -e 'PNG image data' -e 'GIF image data'; then
                     echo "Converting $FILE"
