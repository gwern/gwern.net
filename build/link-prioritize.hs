...
Date: 2019-11-22
When:  Time-stamp: "2026-05-03 13:13:57 gwern"
License: CC-0
...
Links can be extracted from Markdown documents with `linkExtractor.hs` but counting is harder: not
appearing in the manual annotation database `/metadata/{me,full}.gtx` doesn't mean a link needs to be
annotated, because it could be one of the good auto-generated links in `/metadata/auto.hs` so we
can't simply `grep $URL {me,full}.gtx` for each URL & `sort --unique | sort --numeric-sort`; the list
...
$ linkExtractor dnb-faq.md | runghc -istatic/build/ ./static/build/link-prioritize.hs
175 : !W
...
$ find ~/wiki/ -name "*.md" -type f -print0 | parallel --null linkExtractor | grep -E '^http' | ./static/build/link-prioritize.hs
