...
// Author: Said Achmiz
// Date: 2020-03-20
// When:  Time-stamp: "2020-03-20 19:50:51 gwern"
// license: PD
...
function injectModeSelector() {
    ...
    addScrollListener(updateModeSelectorState, "updateModeSelectorStateScrollListener");
    GW.scrollState.modeSelector.forEach(element => {
        element.addEventListener("mouseover", () => { unblurModeSelector(); });
    });
}

/*  Blur the mode selector when scrolling; unblur when scroll to page top.

    Called by the ‘updateModeSelectorStateScrollListener’ scroll listener.
    */
function updateModeSelectorState(event) {
    GWLog("updateModeSelectorState");
    ...
    // Blur mode selector when scrolling half a page in any direction.
    if (GW.scrollState.unbrokenDownScrollDistance > window.innerHeight / 2.0) {
        blurModeSelector();
    }

    // Unblur mode selector when scrolling to top of page.
    if (GW.scrollState.lastScrollTop == 0) {
        unblurModeSelector();
    }
}

function blurModeSelector() {
    GWLog("blurModeSelector");

    GW.scrollState.modeSelector[0].classList.add("blur");
}

function unblurModeSelector() {
    GWLog("unblurModeSelector");

    GW.scrollState.modeSelector[0].classList.remove("blur");
}
