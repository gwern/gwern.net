...
GW.notificationCenter = {
    ...
    /*	Defined event handler phases for certain named events.
    	...
    	GW.contentDidLoad event fires, event handlers are called in the 
    	following order:
        ...
        if (options.phase && phaseOrder) {
            ...
            if (options.phase.startsWith("<")) {
                ...
                for (var j = handlers.length - 1; j > -1; j--) {
                    ...
                    //	TODO: fix the bug that happens here if targetPhaseIndex
                    //	is 0!
                    if (phaseOrder.slice(0, targetPhaseIndex - 1).includes(phaseAt(j).phase)) {
                        j++;
                        break;
                    }
                }
                /*	If neither of the break conditions in the loop were 
                	encountered, j is now equal to -1 [TODO: WHICH IS WRONG!!!],
                	and the new handler will be inserted before the last element
                	of the array [TODO: WHICH IS WRONG!!!].
                	Otherwise, it’ll be inserted in the appropriate place.
                 */
                insertAt = j;
            } else {
    ...
    /*	Fire an event with the given name and event info dictionary.
    	...
    	dictionary should contain is the ‘source’ key, whose value should be a
    ...
    fireEvent: (eventName, eventInfo) => {
        ...
        for (let i = 0; i < GW.notificationCenter.eventHandlers[eventName].length; i++) {
        	...
        	/*	If the handler options specified a true value for the ‘once’ 
        		...
        		(Note that in the case of a once-only handler that’s called 
