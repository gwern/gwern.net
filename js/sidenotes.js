...
function updateTargetCounterpart() {
	...
	} else if (location.hash.match(/#fnref[0-9]/) && window.matchMedia("only screen and (max-width: 176ch)").matches == false) {
...
function expandCollapseBlocksToReveal(element) {
	...
	/*	Update sidenote positions only if we do NOT have to do any further
		...
		*/
	if (!expandCollapseBlocksToReveal(collapseParent.parentElement))
		setTimeout(updateSidenotePositions);

	//	Report that we had to expand a collapse block.
	return true;
}
...
function updateFootnoteReferenceLinks() {
	...
	for (var i = 0; i < GW.sidenotes.footnoteRefs.length; i++) {
		let fnref = GW.sidenotes.footnoteRefs[i];
		if (window.matchMedia("only screen and (max-width: 176ch)").matches == false) {
			fnref.href = "#sn" + (i + 1);
		} else {
			fnref.href = "#fn" + (i + 1);
		}
...
function updateFootnoteEventListeners() {
	...
	/*	Determine whether we are in sidenote mode or footnote mode.
		*/
	var sidenotes = (window.matchMedia("only screen and (max-width: 176ch)").matches == false);
...
function updateSidenotePositions() {
	...
	/*	If we're in footnotes mode (i.e., the viewport is too narrow), then
		don't do anything.
		*/
	if (window.matchMedia("only screen and (max-width: 176ch)").matches == true)
		return;
	...
	for (var i = 0; i < GW.sidenotes.footnoteRefs.length; i++) {
		...
		/*	If the sidenote is cut off (due to being too tall), compensate
			...
			*/
		if (sidenote.firstElementChild.classList.contains("cut-off")) overlap -= GW.sidenotes.sidenoteSpacingAdjustmentForCutOffSidenotes;
		...
		if (previousSidenote && previousSidenote.firstElementChild.classList.contains("cut-off")) headroom += 20;
...
function sidenotesSetup() {
	...
	/*	The `sidenoteSpacing` constant defines the minimum vertical space that
		...
		The `sidenoteSpacingAdjustmentForCutOffSidenotes` constant defines by
		how much to reduce the sidenote spacing to maintain visual consistency
		with sidenotes which are cut off (due to being too tall).
		*/
	GW.sidenotes = {
		sidenoteSpacing:								60,
		sidenoteSpacingAdjustmentForCutOffSidenotes:	20
	};
	...
	/*	Lay out the sidenotes immediately.
		*/
	updateSidenotePositions();
	...
	/*	Listen for changes to whether the viewport width media query is matched;
		...
		*/
	window.matchMedia('(max-width: 176ch)').addListener(GW.sidenotes.viewportWidthBreakpointChanged = () => {
	...
	/*	If the page was loaded with a hash that points to a footnote, but
		...
		*/
	if (location.hash.match(/#sn[0-9]/) &&
		window.matchMedia("only screen and (max-width: 176ch)").matches == true) {
		location.hash = "#fn" + location.hash.substr(3);
	} else if (location.hash.match(/#fn[0-9]/) &&
		window.matchMedia("only screen and (max-width: 176ch)").matches == false) {
